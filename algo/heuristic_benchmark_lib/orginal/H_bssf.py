...
class MaxRects_BSSF:
    ...
    def find_pos_bssf(self, rect):
        ...
        for free_rect in self.free_rectangles:
            ...
            if rect.original_width <= free_rect['width'] and rect.original_height <= free_rect['height']:
                score = self.score_bssf(rect.original_width, rect.original_height, free_rect)
                if score < best_score:
                    best_node = {
                        'x': free_rect['x'],
                        'y': free_rect['y'],
            ...
            if rect.original_height <= free_rect['width'] and rect.original_width <= free_rect['height']:
                score = self.score_bssf(rect.original_height, rect.original_width, free_rect)
                if score < best_score:
                    best_node = {
                        'x': free_rect['x'],
                        'y': free_rect['y'],
    ...
    def insert(self, rect):
        best_node = self.find_pos_bssf(rect)
        if best_node:
...
def bssf_2d_csp(given_rectangles, sheets, verbose=True):
    rectangles = []
    rid = 0
