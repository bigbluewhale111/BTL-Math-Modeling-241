...
class MaxRects_BL:
    ...
    def score_bl(self, x, y):
        return (y, x)

    def find_pos_bl(self, rect):
        best_node = None
        ...
        for free_rect in self.free_rectangles:
            ...
            if rect.original_width <= free_rect['width'] and rect.original_height <= free_rect['height']:
                score = self.score_bl(free_rect['x'], free_rect['y'])
                if score < best_score:
                    best_node = {
                        'x': free_rect['x'],
                        'y': free_rect['y'],
    ...
    def insert(self, rect):
        best_node = self.find_pos_bl(rect)
        if best_node:
    ...
    for rect in rectangles_sorted:
        ...
        if not placed:
            ...
            for bin_width, bin_height, bin_original_index in bins_sorted:
                ...
                bin_pack = MaxRects_BL(bin_width, bin_height)
                if bin_pack.insert(rect):
...
def bl_2d_csp(given_rectangles, sheets, verbose=True):
    rectangles = []
    rid = 0
