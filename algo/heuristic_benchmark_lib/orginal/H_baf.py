...
class MaxRects_BAF:
    ...
    def find_pos_baf(self, rect):
        ...
        for free_rect in self.free_rectangles:
            ...
            if rect.original_width <= free_rect['width'] and rect.original_height <= free_rect['height']:
                score = self.score_baf(rect.original_width, rect.original_height, free_rect)
                if score < best_score:
                    best_node = {
                        'x': free_rect['x'],
                        'y': free_rect['y'],
            ...
            if rect.original_height <= free_rect['width'] and rect.original_width <= free_rect['height']:
                score = self.score_baf(rect.original_height, rect.original_width, free_rect)
                if score < best_score:
                    best_node = {
                        'x': free_rect['x'],
                        'y': free_rect['y'],
    ...
    def insert(self, rect):
        best_node = self.find_pos_baf(rect)
        if best_node:
...
def pack_rectangles(rectangles, bins, verbose=False):
    ...
    for rect in rectangles_sorted:
        ...
        # Try to insert the rectangle into already used bins
        for bin_index, bin_pack in existing_bin_packs:
            if bin_pack.insert(rect):
                rect.bin_index = bin_index 
                placed = True
                break
        # If can't placed, find the next best bin
        if not placed:
            # Try to place in the smallest unused bin
            for bin_width, bin_height, bin_original_index in bins_sorted:
                # Check and skip if bins are already used 
                ...
                bin_pack = MaxRects_BAF(bin_width, bin_height)
                if bin_pack.insert(rect):
                    bin_packs.append((bin_original_index, bin_pack))
                    rect.bin_index = bin_original_index
                    placed = True
                    break
                else:
                    pass
    ...
    # If there are products left, if yes, the algorithm failed
...
def baf_2d_csp(given_rectangles, sheets, verbose=True):
    rectangles = []
    rid = 0
